...
public sealed class WindowsGlobalHotkeyService : IGlobalHotkeyService
{
    ...
    public void Start()
    {
        ...
        _thread = new Thread(MessageLoop) { IsBackground = true, Name = "CloudShot.Hotkeys" };
        _thread.SetApartmentState(ApartmentState.STA);
        _thread.Start();
        _ready.Wait();
        if (_startupError is not null)
        {
            throw new InvalidOperationException("Unable to register CloudShot screenshot hotkeys.", _startupError);
        }
    }
    ...
    private void MessageLoop()
    {
        try
        {
            string className = $"CloudShotHotkeyWindow_{Environment.ProcessId}";
